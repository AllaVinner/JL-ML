# -*- coding: utf-8 -*-
"""
Created on Sat Jul  3 20:02:55 2021
...
class LatentPlaneGUI:
    def __init__(self, decoder,
        ...
        'latent_origin', where the former dictates in which direction the
        ...
        self.latent_origin = latent_origin if latent_origin is not None else np.zeros(self.latent_dim)
        self.latent_point = self.latent_origin
    ...
    def draw(self):
        self.fig.axes[0].cla()
        self.fig.axes[0].set_xlim(-1, 1)
        self.fig.axes[0].set_ylim(-1, 1) 
    ...
    def update_digit(self):
        self.latent_point = self.latent_origin + self.latent_vectors.T @ self.component_point
        self.image = self.decoder.predict(np.array([self.latent_point]))[0]
